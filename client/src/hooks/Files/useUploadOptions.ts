...
export default function useUploadOptions() {
  ...
  const { provider, tools, visionEnabledByAgent } = useAgentToolPermissions(
    agentId,
    ephemeralAgent,
  );
  ...
  /** Model spec says the model reads images, or the agent forces vision on. */
  const visionAvailable = useVisionModel() || visionEnabledByAgent;
