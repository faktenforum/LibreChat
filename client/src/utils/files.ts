...
export type UploadOptionContext = {
  ...
  /**
   * Whether the active model/agent can actually read images. Only images are affected:
   * attaching one to a text-only model is rejected by the provider, so the option is
   * withheld. Undefined means "unknown", which stays permissive.
   */
...
export const getViableUploadOptions = (
  ...
  const withholdImagesFromProvider =
    ctx.visionAvailable === false && types.some((type) => type?.startsWith('image/'));
  if (!withholdImagesFromProvider && every((type) => isProviderAttachType(type, ctx))) {
