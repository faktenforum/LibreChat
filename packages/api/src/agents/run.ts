 ...
 */
function determineVisionCapability(agent: RunAgent, modelSpecs?: TSpecsConfig): boolean {
...
>): Promise<Run<IState>> {
  ...
  const buildAgentInput = (agent: RunAgent, opts: { isSubagent?: boolean } = {}): AgentInputs => {
    ...
    const isValidMaxTokens =
      typeof rawMaxTokens === 'number' && !Number.isNaN(rawMaxTokens) && rawMaxTokens >= 1;
    if (isValidMaxTokens) {
    ...
    const visionCapability = determineVisionCapability(agent, modelSpecs);
